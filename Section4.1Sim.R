# Section3.1Sim.R
# This R code runs the simulations described in Section 3.1.
...
# Simulation Settings
tau2 = 0.01 # 0.01,0.1 # Setting tau2 to 0.1 gives the results used for Table B1
m = 25 # m = 25, 27
...
L = 5 # cap on path length (try L = 4,5,6, L = 4,6 to appendix)
...
for(iter in 1:n_sim){
  ...
  # bib_alt = design.bib(trt = 1:num_edges, k = L, r = (num_edges-1)*L, seed = 123, kinds = "Mersenne-Twister")
  bibd_namestr = paste("BIBDv",num_edges,"b",num_edges*(num_edges-1),
                       "k",L,".csv",sep = "")
  bib_alt = read.csv(bibd_namestr, header = TRUE)
  blockdesign = apply(bib_alt, c(1,2), function(x) as.numeric(substring(x,2))   )
  # blockdesign = t(combn(num_edges, L)) # this lists all possible combinations. It is a BIBD, but it is very large.
...
setwd("./SimResults")
namestr = paste("simres3.1_m",m,"_n",n,"_p",p,"num_edges",num_edges,"tau2",tau2,".csv",sep="")
